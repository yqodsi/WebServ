...
Response::Response() : _statusCode(200) , _headers(""){}
...
std::string Response::getDefaultPage()
{
    ...
    <html>\n\
        ...
        <style>\n\
            ...
            .error{font-size: 2.5rem;text-align: center;margin-top: 10%;color:salmon;}\n\
        </style>\n\
        </head>\n\
        <body>\n\
            <div class=\"container\">\n\
...
void    Response::getMethod(std::string _uri)
{
    ...
    if (this->isLocation)
    {
        if (this->isCGI)
        {
            std::ifstream file(this->_location.getLocationFastCgiPass());
            if (file.is_open())
            {
                file.close();   
        ...
        {
            std::ifstream file(this->_server.getRootDir() + this->_request.getTarget() + RINDEX);
            if (file.is_open())
                get_body(this->_server.getRootDir() + this->_request.getTarget() + RINDEX);
            else
                this->_body = getDefaultPage();
        }
    }
    else
        get_body(_uri);
